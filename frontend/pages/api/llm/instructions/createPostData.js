...
const fetchAllPosts = async () => {
    try {
        const uri = "mongodb+srv://chrisForged:KamrynnLainie0342Y864B8T420C@forgedmartdata.wccoxte.mongodb.net/ForgedMartData";
...
const generateTaskData = async function handleCreatePost(allCategories) {
    ...
    try {
        ...
        const TASK = ` ${allCategories.title} is a list of category titles for a blogging app. Your task is to create comprehensive blog article for each title in the ${allCategories.title} list.
    ...
    <Task Example>
    <Task>
    Write informative and compelling articles based on each title in the ${allCategories.title} list.
    </Task>
    <Instructions>
    You are an AI assistant with a passion for creative writing and storytelling. Your tasks is to write five 1,500 to 3000 words article for each title in the ${allCategories.title} list, 
    ...
    - Content of each article must be within the context of the titles in the ${allCategories.title} list.
    ...
    - Include a 'Category' from items in the '${allCategories.title}' list where each article belongs. 
    
    For Example:
    If an idea for an article is derived from 'Learning' item on the ${allCategories.title} list, then the 'Category' for the article is 'Learning'.
...
async function main() {
    try {
        ...
        const genData = generateTaskData(allPosts);
        return genData;
