...
const StepTwo = () => {
  const [steps, setStep] = useState({
    stepsItems: ["Hungry ?", "Download App", "Complete Task", " Order Food"],
    currentStep: 2,
  });
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 md:px-0 flex justify-center">
          ...
          >
            <div className="flex gap-x-3">
              <div className="flex items-center flex-col gap-x-2">
                <div
                  className={`w-8 h-8 rounded-full border-2 flex-none flex items-center justify-center ${steps.currentStep > idx + 1 ? "bg-green-600 border-green-600 font-semibold" : "" || steps.currentStep == idx + 1 ? "border-green-600 font-semibold" : ""}`}
                >
                  <span
                    className={` ${steps.currentStep > idx + 1 ? "hidden" : "" || steps.currentStep == idx + 1 ? "text-green-600 font-semibold" : ""}`}
                  >
                    {idx + 1}
                  </span>
      ...
      <p className="flex items-center justify-end text-xs underline cursor-pointer">
        More Info
        <img
