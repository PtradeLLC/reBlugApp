                        ...
                        <div className="mt-6 w-full flex-1 space-y-1 px-2">
                            {sidebarNavigation.map((item) => (
                                <a key={item.name} href={item.href} className={classNames(
                                    item.current ? 'bg-stone-300 text-gray-900' : 'text-[#424769] hover:bg-stone-400 hover:text-gray-700',
                                    'group flex w-full flex-col items-center rounded-md p-3 text-xs font-medium'
                                )}
                                    aria-current={item.current ? 'page' : undefined}
                                >
                                    <item.icon className={classNames(
                                        item.current ? 'text-gray-700' : 'text-[#232D3F] group-hover:text-white',
                                        'h-6 w-6'
                                    )} aria-hidden="true" />
