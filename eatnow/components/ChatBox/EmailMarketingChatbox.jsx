...
const EmailChatbox = ({ isOpen, onClose, title, askQuestion, textData }) => {
  ...
  useEffect(() => {
    console.log("TextData from EmailChatbox:", textData);
  }, [textData]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // Retrieve the raw data from localStorage
      const storedData = localStorage.getItem("finalResponse");
      const parsedData = storedData ? JSON.parse(storedData) : null;

      console.log("TextData from EMT", parsedData);
      console.log("newQuestion from EMT", newQuestion);

      const response = await fetch("/api/llm_data", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          textData: parsedData, // Send the parsed data from localStorage
          userQuestion: newQuestion,
        }),
      });

      if (!response.ok) {
        throw new Error("Failed to fetch response from LLM DATA");
      }

      const data = await response.json();
      console.log("Data from LLM DATA:", data);
      setUserQuestion(newQuestion);
      setNewResponse(data.assistantResponse || "");
    } catch (error) {
  ...
  //   try {
  //     console.log("TextData from EMT", textData);
  //     console.log("newQuestion from EMT", newQuestion);

  //     // Make a POST request to the API endpoint (to be implemented)
  //     const response = await fetch("/api/llm_data", {
  //       method: "POST",
  //       headers: {
  //         "Content-Type": "application/json",
  ...
  //     setUserQuestion(newQuestion);
  //     setNewResponse(data.assistantResponse || "");
  //     localStorage.setItem("finalResponse", JSON.stringify(data)); //Review this line
  //   } catch (error) {
  //     console.error("Error posting question:", error);
  //     setError("Failed to post question. Please try again later.");
  //   }
