...
const Plan = ({ textData, isOpen }) => {
  ...
  const fetchConnectedProviders = async (userId) => {
    try {
  ...
  const handleConnectSource = async (provider) => {
    try {
      ...
      const result = await nango.auth(provider.integrationId);

      if (!result || !result.connectionId) {
        throw new Error("Missing Connection ID from Nango auth result.");
      }
          ...
          <div className="mt-10">
            ...
            {/* {providers.map((provider) => (
              <div
                ...
                className="relative mt-2 flex flex-wrap"
              >
                <Button
                  onClick={() => handleConnectSource(provider)}
...
//   console.log("InteData:", integrationData);
...
//         body: JSON.stringify({
//           integrationId: provider.integrationId,
//           connectionId: connectionId, // Now correctly sending connectionId
//           userId: user.$id,
//         }),
//       });
